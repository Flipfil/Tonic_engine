...
namespace tonic::graphics
{
	...
	void Shader::SetUniformMat3(const std::string& name, const glm::mat3 val)
	{
		glUseProgram(m_program_id); TONIC_CHECK_GL_ERROR;
		glUniformMatrix3fv(GetUniformLocation(name), 1, GL_FALSE, glm::value_ptr(val)); TONIC_CHECK_GL_ERROR;
	}
	void Shader::SetUniformMat4(const std::string& name, const glm::mat4 val)
	{
		glUseProgram(m_program_id); TONIC_CHECK_GL_ERROR;
		glUniformMatrix4fv(GetUniformLocation(name), 1, GL_FALSE, glm::value_ptr(val));  TONIC_CHECK_GL_ERROR;
	}
